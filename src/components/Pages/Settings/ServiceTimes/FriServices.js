...
class FriServices extends Component {
  constructor(props) {
    super(props);
    this.state = {
      selectedkey: -1,
  ...
  handleCreate(contact) {
    this.setState({
      friDetails: update(this.state.friDetails, {
        $push: [contact]
      })
    });
  }
  ...
  render() {
    const { friDetails } = this.state;
    const { style } = this.props;
    return (
      <div className="friServices border2 serviceDays">
        <div className="st-title">
          <p className="red-color">Friday Services</p>
        ...
        <div className="card">
          <ServiceTScrollbar style={{ height: 252 }}>
            ...
            <div className="card-block">
              {
                friDetails.map((friDetail, i) =>
                  <FriDetail
                    key={i}
                    nameService={friDetail.nameService}
...
class FriDetail extends Component {
  render() {
    const { nameService, onMouseOver, onRemove, style } = this.props;
    return (
      <div className="fri-detail st-card-detail" onMouseOver={onMouseOver} onClick={onMouseOver}>
